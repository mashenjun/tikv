...
use tikv_util::trace::{self, Collector, Span};
...
impl ReporterBuilder {
    ...
    pub fn duration_threshold(&mut self, value: Duration) {
        self.duration_threshold = Some(value);
    }
...
impl Reporter {
    pub fn collect(
        ...
        if let Some(collector) = collector {
            let mut spans = collector.collect(false, self.duration_threshold);
            ...
            for subscriber in &self.subscribers {
                subscriber.report(
                    if trace_context.get_is_enabled() {
...
pub trait Subscriber: Send + Sync + 'static {
    fn report(&self, trace_id: u64, root_parent_span_id: u64, spans: &[Span]);
}
